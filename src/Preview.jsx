...
function Preview() {
  ...
  useEffect(() => {
    ...
    if (id) {
      ...
      setErrorMsg("URL da ID topilmadi.");
      setLoading(false);
    }
  }, []);
  ...
  const fetchInvitation = async (id) => {
    try {
      ...
      if (error) {
        setErrorMsg("Ma'lumot olishda xato: " + error.message);
      } else {
        setData(data);
        handlePreviewLogic(data);
      }
    } catch (err) {
      setErrorMsg("Tizim xatosi yuz berdi.");
    }
    setLoading(false);
  };

  const handlePreviewLogic = async (invData) => {
    // Agar admin tasdiqlagan bo'lsa, hech narsa qilma
    if (invData.is_active) return;

    // Agar 3 marta ko'rilgan bo'lsa, bloklash
    ...
    await supabase
      .from("v2_invitations")
      .update({ preview_count: newCount })
      .eq("id", invData.id);
      
    // Ekranda darhol yangi sonni ko'rsatish uchun local stateni yangilaymiz
    setData({ ...invData, preview_count: newCount });
  };

  if (loading) return <div style={{ textAlign: "center", marginTop: "50px" }}>Yuklanmoqda...</div>;
  
  // Agar xatolik bo'lsa, foydalanuvchiga ko'rsatamiz (oq sahifa emas!)
  if (errorMsg) return <div style={{ textAlign: "center", marginTop: "50px", color: "red", fontWeight: "bold" }}>{errorMsg}</div>;
  ...
  if (isBlocked) {
    return (
      <div style={{ maxWidth: "500px", margin: "50px auto", background: "white", padding: "30px", borderRadius: "12px", boxShadow: "0 4px 6px rgba(0,0,0,0.1)", textAlign: "center" }}>
        ...
        <p style={{ margin: "20px 0", color: "#555" }}>Taklifnomangizni doimiy ochiq qilish uchun aktivlashtirishingiz kerak.</p>
        <p style={{ fontWeight: "bold" }}>To'lovni quyidagi raqamga qiling va chekni botga yuboring:</p>
        <p style={{ fontSize: "24px", color: "blue", margin: "20px 0" }}>+998 XX XXX XX XX</p> 
        <a 
  ...
  // XAVFSIZLIK: data?.user_data deb yozdik, agar ma'lumot bo'lmasa xato bermaydi
  return (
    <div style={{ maxWidth: "600px", margin: "0 auto", background: "white", minHeight: "100vh", boxShadow: "0 0 10px rgba(0,0,0,0.1)" }}>
      <div style={{ padding: "40px 20px", textAlign: "center" }}>
        ...
        <h1 style={{ color: "#333", fontSize: "28px", margin: "0 0 10px" }}>
          {data?.user_data?.groomName || "Ism"} & {data?.user_data?.brideName || "Ism"}
        </h1>

        <div style={{ width: "50px", height: "2px", background: "#e5e7eb", margin: "20px auto" }}></div>

        <p style={{ color: "#666", fontSize: "18px", margin: "10px 0" }}>
          📅 {data?.user_data?.date || "Sana kiritilmagan"}
        </p>

        <p style={{ color: "#666", fontSize: "18px", margin: "10px 0" }}>
          📍 {data?.user_data?.location || "Manzil kiritilmagan"}
        </p>
        ...
        <p style={{ marginTop: "40px", fontSize: "12px", color: "#9ca3af" }}>
          Qolgan bepul ko'rishlar: {Math.max(0, 3 - (data?.preview_count || 0))} / 3
        </p>
