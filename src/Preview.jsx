...
// --- YORDAMCHI KOMPONENTLAR (Tashqariga chiqarildi) ---

const Petals = () => (
  <div className="pointer-events-none fixed inset-0 z-10 overflow-hidden">
    {Array.from({ length: 16 }).map((_, i) => (
      <span key={i} className="petal absolute rounded-full" style={{ left: `${(i * 53) % 100}%`, width: 5 + (i % 5) * 2, height: 5 + (i % 5) * 2, background: "radial-gradient(circle, oklch(0.92 0.12 80) 0%, oklch(0.78 0.11 70 / 0.4) 60%, transparent 100%)", animationDelay: `-${(i * 1.7) % 12}s`, animationDuration: `${16 + (i % 7) * 2}s` }} />
    ))}
  </div>
);

const Ornament = ({ symbol = "✦" }) => (
...
function Preview() {
  ...
  if (loading) return <div className="text-center mt-[200px] text-white">Yuklanmoqda...</div>;
  if (errorMsg) return <div className="text-center mt-[200px] text-red-500 font-bold">{errorMsg}</div>;

  if (isBlocked) {
    return (
      <div className="max-w-lg mx-auto mt-20 bg-[#111] p-10 rounded-3xl border border-[var(--gold)]/40 text-center backdrop-blur text-white">
        <h2 className="text-3xl font-display text-[var(--gold)]">3 ta bepul ko'rish tugadi!</h2>
        <p className="mt-4 text-gray-400">Taklifnomangizni ochiq qilish uchun aktivlashtiring.</p>
        <p className="mt-6 font-bold text-white">To'lovni qiling va chekni botga yuboring:</p>
  ...
  return (
    <main className="relative overflow-hidden bg-[#0a0a0a] text-[#e5e5e5]">
      <audio ref={audioRef} src={musicUrl} loop preload="auto" />
      
      <div className="fixed left-4 top-4 z-50 flex gap-1.5">
        {["uz", "ru", "en"].map((l) => (<button key={l} onClick={() => setLang(l)} className={`h-9 w-11 rounded-full border text-xs tracking-wider backdrop-blur transition ${lang === l ? "border-[var(--gold)] bg-[var(--gold)]/15 text-[var(--gold)]" : "border-white/10 bg-black/60 text-gray-400"}`}>{l.toUpperCase()}</button>))}
      </div>

      <button onClick={togglePlay} className="fixed right-4 top-4 z-50 grid h-11 w-11 place-items-center rounded-full border border-[var(--gold)]/40 bg-black/70 text-[var(--gold)] shadow-lg backdrop-blur">
      ...
      <section className="relative flex min-h-screen items-center justify-center px-6 py-24 text-center">
        ...
        <div className="relative z-20 mx-auto max-w-xl">
          <Ornament symbol="—" />
          <p className="mt-4 text-[11px] tracking-[0.4em] text-gray-400">{t.invitation}</p>
          <h1 className="mt-10 font-display text-5xl leading-tight text-white md:text-7xl">
          ...
          <p className="mt-10 text-3xl text-[var(--gold)] md:text-4xl" style={{ fontFamily: 'Amiri, serif' }}>وَأَلَّفَ بَيْنَ قُلُوبِهِمْ</p>
          <p className="mt-4 italic text-gray-300">"{t.quote}"</p>
          <p className="mt-1 text-xs tracking-wider text-gray-500">{t.quoteRef}</p>
          <div className="mt-6"><Heart /></div>

          {!unlocked && (
            <button onClick={unlock} className="group relative mt-10 inline-flex items-center gap-3 rounded-full border border-[var(--gold)]/50 bg-black/40 py-3 pl-3 pr-6 backdrop-blur transition hover:border-[var(--gold)]">
              <span className="grid h-10 w-10 place-items-center rounded-full bg-[var(--gold)] text-black">
              ...
              <span className="text-sm tracking-[0.3em] text-white">{t.unlock}</span>
            </button>
          )}
        </div>
      </section>
        ...
        <div id="content">
          <Section className="px-6 py-20">
            <div className="mx-auto max-w-xl text-center">
              ...
              <p className="mt-4 text-base leading-relaxed text-gray-300 md:text-lg">{t.invite}</p>
            </div>
          </Section>
          ...
          <Section className="px-6 py-20">
            <div className="mx-auto max-w-xl text-center">
              ...
              <p className="mt-3 font-display text-5xl tracking-wider text-white md:text-6xl">{dateStr}</p>
                ...
                <div className="mt-10">
                  ...
                  <div className="mt-8 grid grid-cols-4 gap-2 md:gap-5">
                    {[{ v: countdown.days, l: t.days }, { v: countdown.hours, l: t.hours }, { v: countdown.minutes, l: t.minutes }, { v: countdown.seconds, l: t.seconds }].map((it, i) => (
                      <div key={i} className="rounded-2xl border border-[var(--gold)]/25 bg-black/50 p-3 backdrop-blur md:p-5">
                        <div className="gold-text font-display text-3xl md:text-5xl" suppressHydrationWarning>{String(it.v).padStart(2, '0')}</div>
                        <div className="mt-1 text-[10px] tracking-widest text-gray-500 md:text-xs">{it.l}</div>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          <Section className="px-6 py-20">
            <div className="mx-auto max-w-2xl text-center">
              ...
              <div className="mt-8"><Heart /></div>
              <div className="mt-10 rounded-3xl border border-[var(--gold)]/25 bg-black/50 p-8 backdrop-blur">
                <p className="mt-4 text-gray-300 italic">{t.welcome}</p>
              </div>
            </div>
          </Section>
          ...
          <Section className="px-6 py-24">
            <div className="mx-auto max-w-xl text-center">
              ...
              <p className="mt-4 font-display text-3xl italic text-white md:text-4xl">{t.closingDesc}</p>
          ...
          <footer className="relative border-t border-white/10 py-10 text-center text-xs tracking-widest text-gray-500">
            {!data?.is_active && (
               <p className="mb-4 text-[var(--gold)]">Qolgan bepul ko'rishlar: {Math.max(0, 3 - (data?.preview_count || 0))} / 3</p>
            )}
